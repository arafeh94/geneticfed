...
print(np.array([i.y[0] for _, i in clients_data.items()]).reshape(-1, 10))

kmeans = KMeans(10).fit([v for v in client_weights.values()])
print(kmeans.labels_.reshape(-1, 10))

kmeans = KMeans(10).fit([v for v in client_weights_c.values()])
print(kmeans.labels_.reshape(-1, 10))
